...
void GPUtoGPUviaHost(int rank, double *hA, double *dA, int N, double &timer)
{
    ...
    if (rank == 0) {
        ...
        HIP_ERRCHK(hipMemcpy(hA, dA, N * sizeof(double), hipMemcpyDefault));
        ...
        HIP_ERRCHK(hipMemcpy(dA, hA, N * sizeof(double), hipMemcpyDefault));
    } else if (rank == 1) {
        ...
        LAUNCH_KERNEL(add_kernel, N, 1, 0, 0, dA, N);
